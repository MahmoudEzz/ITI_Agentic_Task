...
| Per-agent tool allow-lists, real and enforced (`dispatchTool.js`'s `ToolNotAllowedError`, not just an absence of code that happens not to call a tool); opaque candidate handles (no name) passed to the Rubric Scorer; no destructive/write tool without the approval gate | Excessive agency | Implemented (allow-lists); the approval gate itself lands with issue #41 |
