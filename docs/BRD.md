...
| BR-21 | The provider abstraction supports real token-by-token streaming, not just single-shot completion, with a documented fallback behavior for a stream that fails mid-way (FR-6) | `LLMProviderPort.stream()`; real `OllamaProvider`/`GeminiProvider` implementations (Ollama's `AbortableAsyncIterator`, Gemini's `generateContentStream`); `FallbackLLMProvider.stream()` only fails over before the first delta reaches the caller, propagating a later failure directly rather than silently restarting mid-stream (ADR-0007); `createTracingLLMProvider.stream()` traces a streamed call the same way as `complete()`. Live-verified: a real Ollama stream yields multiple real incremental deltas and a real `done` event with real token counts | Implemented |
| BR-22 | The Q&A and screening workflows are exposed over HTTP as SSE streams — prose deltas for free-text generation, discrete progress events for schema-constrained pipeline steps (FR-6/FR-7) | `POST /ask` streams `delta`/`answer` SSE events via `answerQuestionStream`; `POST /runs` streams `progress` events (`<span>.started`/`.completed`/`.failed`, sourced from the same `recordSpan` `onEvent` hook that persists `trace_events`, not a second mechanism) then one `result`/`error` event; `GET /runs` lists runs, ownership-scoped identically to `GET /runs/:id`. Live-verified against a really-running server: a real streamed `/ask` answer with real incremental deltas and a resolved citation; a real `/runs` call showing real per-attempt retry events and a real `candidate.extract_redact_score.failed` error path (a live grounding-check rejection, not a synthetic one) | Implemented (cancellation and a multipart `/ingest` upload route are explicitly deferred, not silently dropped — see `docs/SECURITY.md`) |
| BR-23 | A minimal web UI demonstrates ingest/ask/run-workflow/approval/trace, served by the same API, and the API reports real readiness, not just liveness (FR-7/FR-9) | `src/adapters/web/public/{index.html,app.js,style.css}` — plain HTML/vanilla JS, no build step, served under `/app`; consumes `/ask` and `/runs`' real SSE responses via `fetch()` + `ReadableStream` (browser `EventSource` can't send POST/auth headers); Ingest tab states plainly that upload is CLI-only rather than faking a button. `GET /readyz` checks real Postgres connectivity (hard-fails if down) and real Ollama reachability (reported as its own degraded dimension, short timeout, never fails the whole probe on Ollama's own documented latency variance) | Implemented |
...
| BR-21 | Implemented | `src/application/ports/LLMProviderPort.js`; `src/adapters/llm/{OllamaProvider,GeminiProvider,FallbackLLMProvider}.js`; `src/application/tracing/createTracingLLMProvider.js`; `tests/unit/{fallbackLLMProvider,createTracingLLMProvider}.test.js`; `tests/integration/{ollamaProvider,geminiProvider}.test.js`; `docs/adr/0007` |
| BR-22 | Implemented (cancellation, multipart /ingest deferred) | `src/adapters/http/routes/{ask,runs}.js`; `src/application/use-cases/answerQuestion.js`'s `createAnswerQuestionStreamUseCase`; `src/application/workflows/runScreeningWorkflow.js`'s `onEvent`; `src/application/tracing/recordSpan.js`'s `onEvent` hook; `tests/unit/{answerQuestion,runScreeningWorkflow,recordSpan}.test.js`; `docs/adr/0007`; `docs/SECURITY.md` |
| BR-23 | Implemented | `src/adapters/web/public/`; `src/adapters/http/server.js`'s `/readyz`; `tests/integration/httpSecurity.test.js` |
