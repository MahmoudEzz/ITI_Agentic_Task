...
| OCR completeness on low-contrast scanned CVs (Phase 5) | Partial — all 5 scanned-CV fixtures OCR successfully (no crashes, no false-positive silent ingestion) and the confidence-threshold system correctly routes them (see below), but actual extracted *content* on 4 of the 5 is limited to the highest-contrast line of the page (name/title/email), not the full CV body | These fixtures are deliberately rendered as low-contrast (dark-gray-on-near-black) scan simulations; tesseract's per-page confidence is a mean over only the words it actually detects as text, so a page can score `confident` (cv-007/014/022: 77/86/77) while most of its body text sits at a local contrast tesseract never segments as text at all. A global per-pixel contrast-stretch was tried and tested against all 5 fixtures — it did not increase how much of any page was detected and measurably hurt cv-002 (61→0) — so it was not adopted (see ADR-0004's Consequences for the full numbers) | The Evidence Extractor genuinely receives less to work with for these 5 candidates than for a native-text CV — this is disclosed, not hidden, and is exactly the scenario the `low_confidence`/`unusable` thresholds and human-review gate exist for; cv-025b (0 confidence, 0 usable text) correctly never gets indexed at all rather than being silently trusted | ~2-4h to add real image preprocessing (adaptive/local thresholding rather than a single global stretch, e.g. via a small image-processing library) if higher OCR completeness on this specific fixture style is later required; out of scope for the assessment's time budget given the confidence-threshold system already handles the risk honestly |
| `/ask` single-shot citation reliability on the small local model (Phase 8, `npm run eval` against the golden set) | No — measured refusal correctness of 45.5% (10/22 auto-scored cases), with 100% of the 12 misses (retrieval diagnostic confirmed `bestDenseSimilarity` 0.545-0.782 in every case) caused by the model's single-shot answer omitting the required `[n]` citation marker, not by missing or insufficient evidence | A CPU-bound 3B-class local model does not reliably comply with a "always cite with `[n]`" instruction on the first generation attempt; retrieval and grounding are both independently verified correct in every one of these cases (see `docs/EVALUATION.md` Baseline results and Failure analysis #1) | `answerQuestion.js`'s existing behavior (treat an uncited claim as a refusal, per BR-08) is the correct fail-safe in the meantime — it never surfaces an unverifiable claim to a user, it just refuses more often than necessary | ~2-4h for a retry-on-missing-citation loop before falling back to refusal, mirroring the schema-validation retry pattern `runStructuredCompletion` already uses elsewhere (ADR-0005); tracked as a follow-up rather than implemented on the strength of one baseline run |

_Additional rows added here as later phases (PDF-vs-DOCX scope, etc.) surface further deferrals worth recording._
...
**Golden-set "conflicting sources" case reframed against a real corpus constraint, not the plan's literal wording (Phase 8, `corpus/golden-set.json`'s `gs-027`).** The plan assumed two contradictory-but-both-retrievable CVs. Querying the real dev database during golden-set authoring showed `cv-025-mina-abdel-malak-b` is `status: 'needs_ocr'` with zero usable chunks — the same fixture ADR-0004 already documented at 0% OCR confidence — so it was never indexed and there is nothing for a second, contradictory version of the evidence to blend with. Rather than authoring a case against a scenario the corpus can't produce, `gs-027` now asserts the (stronger, structurally-guaranteed) property that a question about this candidate is grounded exclusively in the one retrievable CV. `npm run eval` also deliberately does not invoke the full screening workflow at all (a single run has been observed taking 30s-150s with real stalls — see the gap table above) — so the complementary claim, that `CAND-025` is excluded from screening via `InsufficientEvidenceError` rather than silently scored, is left to Phase 5's existing OCR integration-test coverage rather than duplicated here. The bias name-swap invariance metric follows the same reasoning: it's measured in its own integration test (`tests/integration/rubricScorerNameSwapInvariance.test.js`), not folded into `npm run eval`, for the identical reason.
